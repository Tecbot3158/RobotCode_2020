...
public class SplineMove extends CommandBase {
    ...
    public void initialize() {
        ...
        xPos = 0;
        lastAngle = (float) TecbotSensors.getYaw();
    ...
    private void move() {
        ...
        System.out.println("left encoder: " + RobotContainer.getDriveTrain().getLeftEncoder().getRaw());
        System.out.println("right encoder: " + RobotContainer.getDriveTrain().getRightEncoder().getRaw());
        ...
        deltaDistanceLeft = -(-lastLeftCount * TecbotConstants.K_CHASSIS_ENCODER_TO_METERS) + (RobotContainer.getDriveTrain().getLeftPosition() * TecbotConstants.K_CHASSIS_ENCODER_TO_METERS);
        deltaDistanceRight = (-lastRightCount * TecbotConstants.K_CHASSIS_ENCODER_TO_METERS) + (RobotContainer.getDriveTrain().getRightPosition() * TecbotConstants.K_CHASSIS_ENCODER_TO_METERS);

        deltaMiddleWheel = (-lastMiddleCount * TecbotConstants.K_MIDDLE_WHEEL_ENCODER_TO_METERS) + (RobotContainer.getDriveTrain().getMiddlePosition() * TecbotConstants.K_MIDDLE_WHEEL_ENCODER_TO_METERS);
        ...
        if(xPos < startSwervingPoint)
            RobotContainer.getDriveTrain().drive(power, -axis);
        else{
            ...
            RobotContainer.getDriveTrain().swerveMove(fieldX,fieldY, turn);
        }

        System.out.println("clamped value"+ axis);
        System.out.println("delta"+ deltaAngle);

        lastLeftCount = RobotContainer.getDriveTrain().getLeftPosition();
        lastRightCount = RobotContainer.getDriveTrain().getRightPosition();
        lastMiddleCount = RobotContainer.getDriveTrain().getMiddlePosition();
